...
export const ExplorerTree: React.FC<ExplorerTreeProps> = ({ 
  ...
  const loadTenants = async () => {
    ...
    try {
      ...
      dlog('[ExplorerTree] listTenants response:', response);
      ...
      if (response.success && response.data && response.data.length > 0) {
        ...
        dlog('[ExplorerTree] Empty tenant list returned');
        ...
        dlog('[ExplorerTree] Error response:', errorMsg);
        setError(errorMsg);
  ...
  const loadNamespaces = async (tenantNode: TreeNode, tenantName: string) => {
    try {
      ...
      // Use the reference profile for loading structure, not the current cluster
        ...
        if (selectedStructureProfile) {
          try {
            ...
            if (targetResponse.success && targetResponse.data) {
              ...
              dlog('[ExplorerTree] Cannot validate namespaces in target cluster (no permission) - assuming available');
            }
          } catch (err) {
            dlog('[ExplorerTree] Validation failed, assuming items are available:', err);
          }
        }
  ...
  const loadTopics = async (namespaceNode: TreeNode, tenant: string, namespace: string) => {
    try {
      ...
      dlog(`loadTopics response for ${tenant}/${namespace}:`, response);
      
      if (response.success && response.data) {
        dlog(`Found ${response.data.length} topics`);
        ...
        if (selectedStructureProfile) {
          try {
            ...
            if (targetResponse.success && targetResponse.data) {
              ...
              // If we don't have permission to validate, assume items exist (optimistic)
              dlog('[ExplorerTree] Cannot validate topics in target cluster (no permission) - assuming available');
            }
          } catch (err) {
            dlog('[ExplorerTree] Validation failed, assuming items are available:', err);
          }
  ...
  const handleNodeClick = (node: TreeNode) => {
    dlog('Node clicked:', node.id, 'type:', node.type, 'label:', node.label);
    ...
    if (node.type !== 'topic') {
      ...
      if (!node.expanded && (!node.children || node.children.length === 0)) {
        if (node.type === 'tenant') {
          dlog('Loading namespaces for tenant:', node.label);
          ...
          dlog('Loading topics for namespace:', tenant, namespace);
  ...
  // Filter already-loaded nodes by label. Keeps any branch that matches or has a
  // matching descendant, and force-expands matching branches so hits are visible.
  const filterNodes = (nodes: TreeNode[], needle: string): TreeNode[] => {
    const result: TreeNode[] = [];
    for (const node of nodes) {
      const selfMatch = node.label.toLowerCase().includes(needle);
      const filteredChildren = node.children ? filterNodes(node.children, needle) : undefined;
      const hasChildMatch = !!filteredChildren && filteredChildren.length > 0;
      if (selfMatch || hasChildMatch) {
        result.push({
          ...node,
          expanded: hasChildMatch ? true : node.expanded,
          children: hasChildMatch ? filteredChildren : node.children,
        });
      }
    }
    return result;
    ...
    return (
      <div key={node.id} className="tree-node-container">
        ...
        >
          ...
          {node.unavailable && <span style={{ marginLeft: '4px', fontSize: '10px', color: '#999' }}>⚠️ Not in target</span>}
        </div>
  ...
  return (
    <div className="explorer-tree">
      ...
      {tenants.length > 0 && (
        <div className="explorer-tree-search">
          <input
            type="text"
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            placeholder="Filter tenants / namespaces / topics…"
          />
        </div>
      )}
      <div className="explorer-tree-add">
        <input
          type="text"
          value={manualTenant}
      ...
      <div className="explorer-tree-content">
        {tenants.length === 0 ? (
          <div className="explorer-tree-empty">
            <p>No tenants found. Common Pulsar tenant names to try:</p>
            <ul style={{ fontSize: '12px', textAlign: 'left', marginTop: '0.5rem' }}>
              <li><code>public</code></li>
              <li><code>pulsar</code></li>
            </ul>
            <p style={{ marginTop: '0.5rem', fontSize: '12px' }}>Enter a tenant name above to explore it.</p>
          </div>
        ) : (() => {
          const needle = filter.trim().toLowerCase();
          const visible = needle ? filterNodes(tenants, needle) : tenants;
          if (visible.length === 0) {
            return <div className="explorer-tree-empty"><p>No loaded items match “{filter.trim()}”</p></div>;
          }
          return visible.map(node => renderNode(node));
        })()}
      </div>
    </div>
  );
