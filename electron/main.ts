...
function forwardLog(level: 'log' | 'warn' | 'error' | 'info' | 'debug', args: any[]): void {
  ...
  // Only forward verbose levels to the renderer in dev; always forward warn/error
  // so production users still get diagnostics without the debug spam.
  const isVerbose = level === 'log' || level === 'info' || level === 'debug';
  if (mainWindow && (!isVerbose || isDev)) {
    try {
...
// Tracks which cluster each producer/consumer/reader/streaming-consumer id belongs
// to, so we can tear them down when that specific cluster disconnects.
const resourceClusters = new Map<string, string>();
...
ipcMain.handle('cluster:disconnect', async (_event, clusterId: string): Promise<IPCResponse<void>> => {
  try {
    ...
    // Stop any streaming consumer loops for this cluster before closing the client,
    // otherwise their receive loops keep spinning against a closed client.
    for (const [id, sc] of streamingConsumers.entries()) {
      if (sc.clusterId !== clusterId) continue;
      sc.stopRequested = true;
      try {
        if (sc.receiveLoop) await sc.receiveLoop;
      } catch { /* ignore */ }
      streamingConsumers.delete(id);
    }

    // Drop tracked producers/consumers/readers belonging to this cluster.
    // The client.close() below releases the underlying native handles; here we
    // just clear our references so they don't leak across reconnects.
    for (const [id, reader] of readers.entries()) {
      if (resourceClusters.get(id) !== clusterId) continue;
      try { await reader.close(); } catch { /* already closing with client */ }
      readers.delete(id);
      resourceClusters.delete(id);
    }
    for (const id of Array.from(producers.keys())) {
      if (resourceClusters.get(id) !== clusterId) continue;
      producers.delete(id);
      resourceClusters.delete(id);
    }
    for (const id of Array.from(consumers.keys())) {
      if (resourceClusters.get(id) !== clusterId) continue;
      consumers.delete(id);
      resourceClusters.delete(id);
    }
...
ipcMain.handle('admin:getBrokerVersion', async (_event, clusterId: string): Promise<IPCResponse<string>> => {
  try {
    const connection = connectedClusters.get(clusterId);
    if (!connection) {
      return error(`Cluster ${clusterId} is not connected`);
    ...
    console.log('[Main] Fetching broker version for cluster:', clusterId);
    const version = await connection.admin.getBrokerVersion();
    console.log('[Main] Broker version result:', version);
    return success(version);
  } catch (err) {
    console.error('[Main] Failed to get broker version:', err);
    return error(`Failed to get broker version: ${err instanceof Error ? err.message : String(err)}`);
  }
});
...
async function startConsumerLoop(streamingConsumer: StreamingConsumer): Promise<void> {
  while (!streamingConsumer.stopRequested) {
    try {
      ...
      // Receive and acknowledge in one step (acks the native handle before it's
      // freed; the returned message is already detached from native memory).
      const msg = await streamingConsumer.consumer.receiveAndAck_timeout(1000);
...
ipcMain.handle('producer:create', async (_event, clusterId: string, options: CreateProducerOptions): Promise<IPCResponse<string>> => {
  try {
    ...
    resourceClusters.set(producerId, clusterId);
...
ipcMain.handle('producer:close', async (_event, producerId: string): Promise<IPCResponse<void>> => {
  try {
    ...
    resourceClusters.delete(producerId);
...
ipcMain.handle('consumer:create', async (_event, clusterId: string, options: CreateConsumerOptions): Promise<IPCResponse<string>> => {
  try {
    ...
    resourceClusters.set(consumerId, clusterId);
...
ipcMain.handle('consumer:acknowledge', async (_event, consumerId: string, _messageId: string): Promise<IPCResponse<void>> => {
  try {
    ...
    // No-op: the native binding can't reconstruct a MessageId from its string
    // form, and we must not retain native Message handles across async ticks
    // (that crashes the addon). The streaming consumer path acknowledges inline
    // via receiveAndAck_timeout instead, so explicit ack is unused here.
    return success(undefined);
  } catch (err) {
...
ipcMain.handle('consumer:close', async (_event, consumerId: string): Promise<IPCResponse<void>> => {
  try {
    ...
    resourceClusters.delete(consumerId);
...
ipcMain.handle('messages:browseMessages', async (_event, clusterId: string, topicName: string): Promise<IPCResponse<{ readerId: string }>> => {
  try {
    ...
    resourceClusters.set(readerId, clusterId);
...
ipcMain.handle('messages:closeReader', async (_event, _clusterId: string, readerId: string): Promise<IPCResponse<void>> => {
  try {
    ...
    resourceClusters.delete(readerId);
