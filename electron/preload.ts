...
contextBridge.exposeInMainWorld('lightcurve', {
  ...
  admin: {
    ...
    getBrokerVersion: async (clusterId: string): Promise<IPCResponse<string>> => {
      return await ipcRenderer.invoke('admin:getBrokerVersion', clusterId);
    },
...
export interface LightCurveAPI {
  ...
  admin: {
    ...
    getBrokerVersion: (clusterId: string) => Promise<IPCResponse<string>>;
  ...
  messages: {
    ...
    unsubscribe: (clusterId: string, topic: string) => Promise<IPCResponse<void>>;
  };
  producer: {
