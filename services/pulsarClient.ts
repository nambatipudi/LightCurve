...
export class PulsarConsumer {
  ...
  /**
   * Receive a message and acknowledge it immediately, while the native message
   * handle is still valid. We never retain native Pulsar.Message objects across
   * async boundaries — doing so references freed native memory and crashes the
   * addon (see convertMessage). Returns the converted (handle-free) message.
   */
  async receiveAndAck_timeout(timeoutMs: number): Promise<PulsarMessage | null> {
    try {
      const msg = await this.consumer.receive(timeoutMs);
      const converted = this.convertMessage(msg); // copies data off native memory
...
export class PulsarReader {
  ...
  private convertMessage(msg: Pulsar.Message): PulsarMessage {
    return {
      // Copy the buffer to avoid referencing freed native memory after the
      // message/reader is released. Returning msg.getData() directly hands out a
      // view into native memory that gets freed, so a later .toString() segfaults
      // (this is the same precaution PulsarConsumer.convertMessage takes).
      data: Buffer.from(msg.getData()),
      messageId: msg.getMessageId().toString(),
      properties: { ...(msg.getProperties() || {}) },
      publishTimestamp: msg.getPublishTimestamp(),
