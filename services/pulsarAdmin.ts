...
export class PulsarAdmin {
  ...
  /**
   * Get the Pulsar broker version
   */
  async getBrokerVersion(): Promise<string> {
    try {
      // Try to get version from /admin/v2/brokers/leader-broker
      const brokerInfo = await this.request<any>('/admin/v2/brokers/leader-broker');
      console.log('[PulsarAdmin] Leader broker response:', brokerInfo);
      if (brokerInfo?.version) {
        return brokerInfo.version;
      }
    } catch (error) {
      console.log('[PulsarAdmin] Leader broker endpoint failed:', error);
    }

    try {
      // Try /admin/v2/broker-info or /admin/v2/brokers endpoint to get version
      const brokers = await this.request<any>('/admin/v2/brokers');
      console.log('[PulsarAdmin] Brokers list response:', brokers);
      if (brokers && Array.isArray(brokers) && brokers.length > 0) {
        console.log('[PulsarAdmin] Connected to Pulsar broker successfully');
        return 'Connected to Pulsar';
      }
    } catch (error) {
      console.log('[PulsarAdmin] Brokers endpoint failed:', error);
    }

    try {
      // Fallback: Try to list available clusters to confirm broker is working
      const clusters = await this.listClusters();
      console.log('[PulsarAdmin] Clusters response:', clusters);
      if (clusters && clusters.length >= 0) {
        return 'Connected to Pulsar';
      }
    } catch (error) {
      console.log('[PulsarAdmin] Clusters list failed:', error);
    }

    return 'Pulsar Broker';
  }
